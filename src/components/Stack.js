...
import alex from "../images/xf.png";
...
function Stack() {
  ...
  useEffect(()=>{
    const data=localStorage.getItem("doc1");
    if(data){
    ...
    useEffect(()=>{
        localStorage.setItem("doc1",JSON.stringify(prevMessage));
    },[prevMessage])
  ...
  React.useEffect(() => {
    ...
    return response.json();
            ...
            setPrevMessage(prevMessage=>{
               return  [...prevMessage,
                    {messege : response.chatbot.response,user : false,id:prevMessage.length}
                ]
            });
            }).catch(err => {});
        };
                            ...
                            <p id="botStarterMessage" className="botText">
                                <p className="botText">
                                    <img src={alex} alt="Doctor-Image" className='small-img'/>
                                       <span><div className="msgtxt">Hi, this is EVA</div></span>
                                    </p>
                                </p> 
