...
function Doctor() {
  ...
  useEffect(()=>{
    ...
    useEffect(()=>{
        localStorage.setItem("doc",JSON.stringify(prevMessage));
    },[prevMessage])
  ...
  React.useEffect(() => {
    ...
    const apiFetch1=()=>{
        fetch(`https://acobot-brainshop-ai-v1.p.rapidapi.com/get?bid=178&key=sX5A2PcYZbsN5EY6&uid=mashape&msg=${userMessege}`, {
	"method": "GET",
	"headers": {
    ...
    return response.json();
            ...
            setPrevMessage(prevMessage=>{
               return  [...prevMessage,
                    {messege : response.cnt,user : false,id:prevMessage.length}
                ]
            });
            }).catch(err => {});
        };
                            ...
                            <p id="botStarterMessage" className="botText">
                                <p className="botText">
                                    <img src={alex} alt="Doctor-Image" className='small-img'/>
                                       <span><div className="msgtxt">Hi, this is ACO</div></span>
                                    </p>
                                </p> 
